...
struct letra{
    char info;
    int cont = 0;
    letra <T> *eloleft = NULL, *eloright = NULL;
};
...
template <typename T>
bool inserirLDE(LDE <T> &lista, letra <char> *valor){
    No <T> *novo;
    ...
    // Inser��o no comeco
    if( valor->cont <= lista.comeco->info->cont){
    ...
    // Inser��o no final
    if( valor->cont >= lista.fim->info->cont ){
        lista.fim->eloP = novo;
        novo->eloA = lista.fim;
        lista.fim = novo;
        return true;
    ...
    while( ant->info->cont < valor->cont && ant->eloP->info->cont <= valor->cont )
        ant = ant->eloP;
    No <T> *prox = ant->eloP;
...
No <T> * pesquisarLDE(LDE <T> lista, T valor){
    ...
    while( aux != NULL ){
        if( aux->info->info == valor ) return aux;
        aux = aux->eloP;
    }
    return aux;
}
...
//
template <typename T>
bool retirarLDE(LDE <T> &lista, No <char> *valor ){
    No <T>*aux, *anterior, *proximo;

    aux = pesquisarLDE(lista, valor->info->info);
